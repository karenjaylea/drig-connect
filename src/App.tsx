  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-gray-100 flex items-center justify-center">
        <div className="text-xl text-gray-700">Loading events...</div>
      </div>
    );
  }
                ...
                <div className="flex items-center justify-between mb-6">
                  ...
                  {selectedDate && (
                    <button
                      ...
                      className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 text-lg"
                    >
                      Show All
                    </button>
                  )}
                  ...
                  {filteredEvents.length === 0 ? (
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
                      ...
                      <p className="text-lg text-gray-600">
                        {selectedDate ? 'There are no events scheduled for this date.' : 'There are no upcoming events at the moment.'}
                      </p>
                    </div>
                  ) : (
            ...
            <div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div className="space-y-3">
                  {filteredEvents.length === 0 ? (
                    <p className="text-lg text-gray-500">{selectedDate ? 'No events on this date' : 'No upcoming events'}</p>
                  ) : (
                    filteredEvents.slice(0, 5).map(event => (
                      <button
                        key={event.id}
                        onClick={() => setSelectedEvent(event)}
                        className="w-full text-left p-4 rounded-lg hover:bg-gray-50 transition-colors border border-gray-100 text-lg"
                      >
                        <div className="font-medium text-gray-900 mb-1 text-lg">{event.title}</div>
                        <div className="text-gray-500 text-base">{event.event_time.substring(0, 5)}</div>
                      </button>
                    ))
                  )}
                </div>
                {filteredEvents.length > 5 && (
                  <button
                    ...
                    className="w-full mt-4 text-blue-600 hover:text-blue-700 text-lg font-medium"
                  >
